...
* Собрать ссылки на заметки или заголовки вручную в произвольном порядке и сохранить как шаблон
...
* Встраивать заметки на доску и располагать их в произвольном порядке с показом текста
...
from "folder" and #tag
where !contains(file.inlinks.file.name, "excalidraw")
sort file.name desc
```
